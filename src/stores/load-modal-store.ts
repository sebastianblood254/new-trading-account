...
export default class LoadModalStore {
    ...
    handleFileChange = (
        ...
        this.readFile(false, event as DragEvent, file); // Changed is_preview to false for immediate load
    ...
    onLoadStrategy = async () => {
        const { dashboard } = this.root_store;
        // Constants for notification types usually come from a shared file, 
        // but since we are in the store, we can just call the necessary methods.
        ...
        dashboard.setOpenSettings('BOT_IMPORT'); // 'BOT_IMPORT' is the string value of NOTIFICATION_TYPE.BOT_IMPORT
    };
