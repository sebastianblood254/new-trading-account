...
export const generateOAuthURL = async (prompt?: string) => {
    try {
        ...
        const clientId = process.env.CLIENT_ID || (isProduction() ? DEFAULT_APP_ID.PRODUCTION : DEFAULT_APP_ID.STAGING);
        ...
        if (hostname && clientId) {
            ...
            const appId = process.env.APP_ID || (isProduction() ? DEFAULT_APP_ID.PRODUCTION : DEFAULT_APP_ID.STAGING);
            if (appId) {
